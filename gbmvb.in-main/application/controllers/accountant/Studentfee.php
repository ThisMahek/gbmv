    ...
    function reportbyname() {
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['past_year'] = $this->studentfee_model->get_student_previous_session($student_id);
                $data['current_year']  =$this->setting_model->getCurrentSessionName();
               // echo $this->db->last_query();die();
                  //  echo '<pre>';
            //   print_r( $data['past_year']);exit;
                $this->load->view('layout/accountant/header', $data);
                $this->load->view('accountant/studentfee/reportByName', $data);
                $this->load->view('layout/accountant/footer', $data);
            }
