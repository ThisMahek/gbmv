...
//new modification
 ...
 $fee_discount = 0;
    ...
    //end new modifications
         ...
         $data['fee_discount']=$fee_discount;
        //  print_r($data['fee_discount']);
    ...
    function searchpayment() {
        ...
        $this->form_validation->set_rules('receiptno', 'Receipt No', 'trim|required|xss_clean');
        if ($this->form_validation->run() == FALSE) {
            
        } else {
            $paymentid = $this->input->post('receiptno');
           // print_r($paymentid);exit;
            $invoice = explode("/", $paymentid);
        //  $a=   implode(" ",$invoice);
                //print_r($a);exit;
            if (array_key_exists(0, $invoice) && array_key_exists(1, $invoice)) {
                 $a=substr($invoice[0],3);
                    //print_r($a);exit;
                $invoice_id = $a;
    ...
    function searchpaymentbydate() {
        ...
        $this->form_validation->set_rules('start_date', 'Payment Start Date', 'trim|required|xss_clean');
        $this->form_validation->set_rules('end_date', 'Payment End Date', 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $feeList = $this->studentfeemaster_model->getFeeByDate($start_date, $end_date);
            //$feeList = $this->studentfee_model->getFeeBetweenDate($start_date, $end_date);
            
            $data['feeLists'] = $feeList;
            //$data['sub_invoice_id'] = '2';
            //$data['feeList'] = array();
        }
        $this->load->view('layout/header', $data);
        $this->load->view('studentfee/searchpaymentbetweendate', $data);
        $this->load->view('layout/footer', $data);
    }
