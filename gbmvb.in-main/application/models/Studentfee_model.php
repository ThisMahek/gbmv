...
class Studentfee_model extends CI_Model {
    ...
    public function get_student_previous_session($student_id)
    {
        $this->db->select('sessions.session');
        $this->db->join('sessions','sessions.id=student_session.session_id','left');
      return  $this->db->where('student_session.student_id',$student_id)->order_by('student_session.id','ASC')->get('student_session')->row();
    }
